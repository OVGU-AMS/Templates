
\documentclass[numberofslides]{AMSBeamer}
%% Options: 
%% numberofslides: show total number of slides
%% noAMSLogo: do not show AMS Logo
%% smallfootfontsize: decrease fontsize in footline


%\usepackage{import} %use includefrom instead import to use includeonly
%% include only specific texfile:
% \includeonly{subdirectory/texfile}
%% include only labeled frame:
% \includeonlyframes{title,label}


%% decrease title font if necessary
%\setbeamerfont{title}{series=\bfseries,size=\fontsize{14}{18}}

%% margins for title can also be changed:
% \makeatletter
% \setlength\title@titlemargin{13mm}\setlength\title@institutemargin\title@titlemargin
% \makeatother

%% Title:
\title{Full Title of the Presentation}
%% include [Short Title] if title is too long for footline:
% \title[Short Title]{Full Title of the Presentation}

%% Subtitle if necessary:
\subtitle{Subtitle: Template Version 2024-11-29 for AMS presentations}

%% Author(s)
\author{James T. Kirk}


\institute[Autonomous Multisensor Systems Group]{
	Autonomous Multisensor Systems Group \\
	Institute for Intelligent Cooperating Systems \\
	Faculty of Computer Science \\
	Otto von Guericke University Magdeburg
}
\mode<presentation>{\keywords{Schlüsselwörter durch Komma getrennt}}

\date{XX.YY.ZZZZ}%{Datum der Präsentation, \zB 1. Januar 2016}

\begin{document}


\section{Titel}
\begin{frame}[label=title]
	\maketitle
\end{frame}


\section{Test Slides}

%
% You can structure your slide into parts, sections and subsections
\part{General Information}
%
\section{First Section}
% Use this command for section slides:
\sectionslide{\insertsection}
\sectionslide[Chapter / Part name (Default is title of presentation)]{This is another section}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Slide using TikZ
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Test Slide Using TikZ}

\begin{frame}[label=slidelabel]
	\frametitle{\insertsubsectionhead}%
	\begin{tikzframe}
		%% show grid:
		\makegrid
		\node[anchor=north west,text width=3cm] at (3,4) {width: \SI{160}{mm}\\height: \SI{78}{mm}};
	\end{tikzframe}
    % \transfade<2->
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Slide using textblocks
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Test Slide Using Textblocks}

\begin{frame}[label=slidelabel]
	\frametitle{\insertsubsectionhead}%
	\begin{tikzframe}
		%% show grid:
		\node[anchor=north west,text width=3cm] at (1,7) {tikzframe lies on the background layer};
	\end{tikzframe}
	%
	\makegridtp
	\begin{textblock*}{30mm}(8cm,20mm)%
		You can use textpos instead of tikz
	\end{textblock*}%
	\begin{textblock*}{70mm}(8cm,50mm)%
		for textpos, you can use makegridtp \\
		origin is the upper left corner
	\end{textblock*}%
	%
	\Notes{
		you can add separate todo notes here
    }
    % \transfade<2->
\end{frame}


\section{Further Notes}
\sectionslide{\insertsection}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Slide without tikz or textblock
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection{Test Slide Without TikZ or Textblocks}

\begin{frame}[label=slidelabel]
	\frametitle{\insertsubsectionhead}%
	%
	% \makegridtp


	\bigskip%
	The folder \emph{pages} contains an Adobe Illustrator template to create slides. The slide area is \SI{160}{mm} $\times$ \SI{78}{mm}.
	
	\bigskip
	The following standard colors are provided:\\ \bfseries\textcolor{AMSblue}{AMSblue}, \textcolor{AMSred}{AMSred}, \textcolor{AMSgreen}{AMSgreen}, \textcolor{AMSorange}{AMSorange}

	% \begin{tikzframe}
	% 	% \path<1-> (0,0) node[anchor = south west,inner sep=0pt]{%
	% 	% \includegraphics{./AI_gerated.pdf}};
	% \end{tikzframe}


    % \transfade<2->
\end{frame}

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
% Thank you slide
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begingroup% to remove header and footline and include style of first slide
\setbeamertemplate{header}{}
\setbeamertemplate{footline}{}
\begin{frame}[label=overview]
	\frametitle{Thank You For Your Attention!}%
	\begin{tikzpicture}[overlay, remember picture, shift={(current page.south west)}, inner sep=0pt, outer sep=0pt]

		\useasboundingbox (0,0) rectangle (160mm,90mm);%

		\node[anchor=south east] at (150mm,7mm) {\includegraphics[width=50mm]{logos/AMS.pdf}};%

		\fill[ovgu-inf-blue] (0,90mm) rectangle (160mm,55mm);
		\node[anchor=north east] at (160mm,90mm) {\includegraphics{./logos/otto.pdf}};
		% \makeatletter
		\node[anchor=north west] at (14mm,87mm) {\includegraphics[height=11mm]{./logos/Signet_INF_1_inv.pdf}};	
		% \makeatother
		\node[anchor=south west, text width=160mm-28mm] at (14mm,55mm+5mm) {\color{white}\usebeamerfont{title}\inserttitle\par};

		\node[anchor=north west, text width=160mm-28] at (14mm,55mm-15mm) {%
			{\usebeamerfont{title}\color{AMSblue}\insertframetitle\par}%
		};			

		\node[anchor=south west, text width=160mm-28] at (14mm,7mm) {%
			\usebeamerfont{institute}%
			\textbf{\insertauthor}\par%

			\begin{tabular}[t]{@{}l@{\hspace{3pt}}p{.32\textwidth}@{}}
				Web: & \url{ams.ovgu.de}\\[1mm]
				Mail: & \url{enterprise@ovgu.de} \\[1mm]
				% Phone: & +49 391 67 57580 
			\end{tabular}%
		};			

	\end{tikzpicture}
    % \Notes{

    % }
    \transfade[duration=5]%<2->
\end{frame}	
\endgroup

\end{document}
